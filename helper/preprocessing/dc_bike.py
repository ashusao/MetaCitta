...
def load_csv(data_dir):
    if not glob.glob(os.path.join(data_dir, '*.csv')):
        ...
        for month in range(7, 13):
            urllib.request.urlretrieve(
                "https://s3.amazonaws.com/capitalbikeshare-data/2020{0:0=2d}-capitalbikeshare-tripdata.zip".format(month),
                data_dir + "/2020{0:0=2d}-capitalbikeshare-tripdata.zip".format(month))
            with zipfile.ZipFile(data_dir + \
                                 "/2020{0:0=2d}-capitalbikeshare-tripdata.zip".format(month), "r") as zip_ref:
    ...
    df.dropna(subset=['end_lat', 'end_lng', 'start_lat', 'start_lng'], inplace=True)
    df['started_at'] = pd.to_datetime(df['started_at']).dt.floor('h')
    df['ended_at'] = pd.to_datetime(df['ended_at']).dt.floor('h')
    df.drop(df[(df.start_lat == 0) | (df.end_lat == 0) | (df.start_lng == 0) | (df.end_lng == 0)].index, inplace=True)
...
def preprocess_dc_bike(dir, file_name):
